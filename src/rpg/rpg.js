...
const rpg = {
  ...
  defaultSkillTestUseable:function(char) {
    return !(!this.doesNotRequireManipulate && !char.testManipulate());

  },
  defaultSkillAfterUse:function(attack, count) { },

  defaultSpellTestUseable:function(char) {
    return !(!this.doesNotRequireTalk && !char.testTalk());
  },
...
world.resetPauses = function() {
  ...
  loc().rpgLighting = game.dark ? rpg.DARK : rpg.LIGHT
  if (!loc().rpgFog) loc().rpgFog = 0
  let targetFog = loc().defaultFog ? loc().defaultFog : 0
  if (loc().activeEffects) {
    for (const effectName of loc().activeEffects) {
      const effect = rpg.findEffect(effectName)
      if (effect.fogEffect) targetFog *= effect.fogEffect
      if (effect.lightEffect) {
        if (effect.lightEffect === rpg.UTTERLIGHT) loc().rpgLighting = rpg.UTTERLIGHT
        if (effect.lightEffect === rpg.UTTERDARK && loc().rpgLighting !== rpg.UTTERLIGHT) loc().rpgLighting = rpg.UTTERDARK
        if (effect.lightEffect === rpg.LIGHT && loc().rpgLighting !== rpg.UTTERLIGHT && loc().rpgLighting !== rpg.UTTERDARK) loc().rpgLighting = rpg.DARK
        if (effect.lightEffect === rpg.LIGHT && loc().rpgLighting !== rpg.UTTERLIGHT && loc().rpgLighting !== rpg.UTTERDARK && loc().rpgLighting !== rpg.LIGHT)
            loc().rpgLighting = rpg.DARK
      }
    }
    game.dark = (loc().rpgLighting === rpg.UTTERDARK || loc().rpgLighting === rpg.DARK) // !!! This could have bad consequences!
  }
  if (targetFog > loc().rpgFog) loc().rpgFog++
  if (targetFog < loc().rpgFog) loc().rpgFog--
}
