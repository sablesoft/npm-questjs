...
import {errormsg, player, settings, w} from "../index.js";
...
map.redraw = function(offX, offY) {
  ...
  if (w[plr.loc].mapX) plr.mapX = w[plr.loc].mapX / settings.mapScale
  if (w[plr.loc].mapY) plr.mapY = w[plr.loc].mapY / settings.mapScale
  if (w[plr.loc].mapRegion) plr.mapRegion = w[plr.loc].mapRegion

  if (!plr.mapRegion) plr.mapRegion = settings.mapImages[0].name
  const mapImage = settings.mapImages.find(el => el.name === plr.mapRegion)
  if (!mapImage) return errormsg("Failed to find a map region called '" + plr.mapRegion + "'")
  ...
  const x = plr.mapX + offsetX
  const y = plr.mapY + offsetY
