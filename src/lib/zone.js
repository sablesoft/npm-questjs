...
import {draw, Exit, failedmsg, falsemsg, lang, msg, player, settings, w} from "../index.js";
...
const ZONE = function(defaultToBlocked) {
  ...
  res.beforeEnter = function() {
    ...
    for (let name in w) {
      const o = w[name]
      if (o.zoneElsewhere === this.name && o.positionX === player().positionX &&
          o.positionY === player().positionY) {
        o.loc = this.name
  ...
  // No guarantee what will happen if the char is at the coordinates
  ...
  res.drawMap = function() {
    ...
    map.push('<rect x="' + (x2+4) + '" y="' + (y2+4) + '" width="' + (this.cellSize-8) +
        '" height="' + (this.cellSize-8) + '" stroke="none" fill="' + this.playerColour + '"/>')
...
const ZONE_FEATURE = function(loc, x, y, range, adjacent) {
    return {
      ...
      isLocatedAt: function (loc) {
          if (adjacent) {
              return loc === this.zone && Math.abs(player().positionX - this.x) < 2 &&
                  Math.abs(player().positionY - this.y) < 2
          } else {
